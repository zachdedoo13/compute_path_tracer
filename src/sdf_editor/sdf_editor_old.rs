...
impl SDFEditorOld {
   ...
   pub fn update(&mut self, path_tracer: &mut PathTracer, setup: &Setup, input_manager: &InputManager, comp_data: &mut CompData) {
      ...
      if self.rec_update.queue_update {
         ...
         println!("{:?}", comp_data.data_array.data)
      }
   ...
   fn open(&mut self, name: &String) {
      ...
      let deserialize: SDFEditorOld = from_str(&contents).expect("Failed to deserialize");
...
pub struct SDFEditorPackageOld {
   pub sdfeditor: SDFEditorOld,
   pub comp_data: CompData,
}
impl SDFEditorPackageOld {
   pub fn new(setup: &Setup) -> Self {
      Self {
         sdfeditor: SDFEditorOld::new(),
